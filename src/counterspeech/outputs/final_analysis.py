...
# =========================================================
files = {
    "LLaMA_HUMAN": "llama32_human_reviewed.json",
    "Qwen_HUMAN":  "qwen25_human_reviewed.json",
...
def to_df(entries, model, score_type):
    ...
    for idx, entry in enumerate(entries):
        scores = entry["perspective_scores"]
...
savefig("Scatter_LLaMA_vs_Qwen_HUMAN_Normalised")
...
savefig("Scatter_LLaMA_vs_Qwen_Perspective")
...
def compare_human_persp(df_h, df_p, model):
    ...
    savefig(f"{model}_Human_vs_Perspective")
...
def diff_hist(df_h, df_p, model):
    ...
    savefig(f"{model}_HumanMinusPerspective")
...
def calibration(df_h, df_p, model):
    ...
    plt.ylabel("Score")
    savefig(f"{model}_CalibrationCurve")
...
def disagreement_heatmap(df_h, df_p, model):
    ...
    savefig(f"{model}_HumanPerspective_DisagreementHeatmap")
...
savefig("Perspective_Bias_Drift")
