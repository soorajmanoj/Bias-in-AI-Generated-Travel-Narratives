...
"""
@file analysis.py
@brief Generate exploratory figures comparing Perspective API toxicity across languages.

Loads perspective-scored counterspeech, filters to hinglish and english, computes
means with 95% CI, distribution plots, histograms, boxplots, heatmaps and word clouds.
"""
...
# Rename rom_hindi -> hinglish and keep only hinglish/english
...
def mean_ci(series):
    """
    @brief Compute mean and 95% CI for a pandas Series.

    @param series Numeric pandas Series.
    @return Tuple (mean, ci_low, ci_high).
    """
    mean = series.mean()
